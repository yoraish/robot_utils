...
class BagToImageSyncedTrajDataset(object):
    def __init__(self, bagfile_gp, image_topic, imu_topic, mocap_topic, out_dir_gp = None, bagstart = 0, bagend = np.inf):
        ...
        self.img_ix = 0
        # A pose is only added to the dataset if a frame is received. This pose is the most recent pose from the mocap system. Under the assumption that the rate of the mocap is high.
        self.recent_mocap_pose = None
        ...
        for topic, msg, t in tqdm(bag.read_messages(topics=[self.image_topic, self.imu_topic, self.mocap_topic])):
            ...
            if t.to_sec() - startt > self.bagstart:
                ...
                if topic == self.image_topic:
                    # Write the most recent mocap pose to the dataset.
                    if self.recent_mocap_pose is not None:
                        self.gt_traj_data.append(self.recent_mocap_pose)
                ...
                ##################
                    ...
                    self.recent_mocap_pose = [t.to_nsec(), dx, dy, dz, q[0], q[1], q[2], q[3]]
...
if __name__ == "__main__":
    args = handle_args()
    b2f = BagToImageSyncedTrajDataset(args.bagfile, args.image_topic, args.imu_topic, args.mocap_topic, args.output_root, float(args.start), float(args.end))
    b2f.process_bag()
    # b2f.visualize_traj()
